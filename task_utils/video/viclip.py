...
from task_utils.video.viclip_vision import clip_joint_l14
from task_utils.video.viclip_text import clip_text_l14
logger = logging.getLogger(__name__)

class ViCLIP(nn.Module):
    """docstring for ViCLIP"""

    def __init__(self,  tokenizer=None, pretrain=None, freeze_text=True):
        super(ViCLIP, self).__init__()
        if pretrain is None: 
            pretrain = '/path/to/ViClip-InternVid-10M-FLT.pth'
        if tokenizer:
